...
--
-- DEFENSIVE: every column the view references is ensured to exist first, so the
-- view always recreates successfully no matter which earlier migrations ran.
...
ALTER TABLE accounts
  ...
  ADD COLUMN IF NOT EXISTS statement_day     INTEGER,         -- (from v36) — ensure present
  ADD COLUMN IF NOT EXISTS statement_due_day INTEGER;         -- (from v17) — ensure present

-- Rebuild account_balances to expose the new columns. Balance math is unchanged.
