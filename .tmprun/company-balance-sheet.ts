...
// Company A bills company B. That is ONE event with TWO sides — a receivable
// for A and a payable for B. Before this, only A's side existed: B looked like
// it owed nothing, and the group's books silently didn't balance.
describe('inter-company billing', () => {
  const invoice = 100000
  const ic = {
    accounts: [], assets: [],
    receivables: [{ id: 'r', number: 'A-1', party: 'Company B', companyId: A, outstanding: invoice }],
    payables: [{ id: 'ic-r', number: 'A-1', party: 'Company A', companyId: B, outstanding: invoice, interCompany: true }],
  }

  it('shows the money owed TO the seller', () => {
    expect(buildBalanceSheet(A, ic).receivables).toBe(invoice)
    expect(buildBalanceSheet(A, ic).net).toBe(invoice)
  })

  it('shows the matching debt ON the buyer — this is the half that was missing', () => {
    expect(buildBalanceSheet(B, ic).payables).toBe(invoice)
    expect(buildBalanceSheet(B, ic).net).toBe(-invoice)
  })

  it('cancels out across the group, because no money left the group', () => {
    const a = buildBalanceSheet(A, ic).net
    const b = buildBalanceSheet(B, ic).net
    expect(a + b).toBe(0)
  })

  it('flags the debt as inter-company, not money owed to the outside world', () => {
    expect(ic.payables[0].interCompany).toBe(true)
  })
})
