...
export async function GET(req: NextRequest) {
  ...
  const { data, error } = await supabase.from('chit_collections')
    .select('*, member:chit_members(name)').eq('user_id', owner).eq('group_id', groupId)
...
export async function POST(req: NextRequest) {
  ...
  const txn = collectionTransaction({
    userId: owner, accountId, amount, date,
  ...
  const row = {
    user_id: owner, group_id: groupId, member_id: memberId, month_number: monthNumber,
  ...
  await supabase.from('chit_receivables')
    ...
    .eq('user_id', owner).eq('group_id', groupId).eq('member_id', memberId).eq('month_number', monthNumber)
...
export async function DELETE(req: NextRequest) {
  ...
  const owner = access.ownerId
  if (!CAN.recordCollection(access.role)) {
    return NextResponse.json({ error: forbidden('reverse collections', access.role) }, { status: 403 })
  }
  // transactions / categories keep their OWNER-ONLY policies: a staff session
  // cannot touch the general ledger. The posting below is done on the owner's
  // behalf with an elevated client, and only after the grant and the role have
  ...
  if (coll.income_transaction_id) {
    const { data: siblings } = await supabase.from('chit_collections')
      ...
      .eq('income_transaction_id', coll.income_transaction_id).eq('user_id', owner)
      .neq('id', coll.id)
    ...
    if ((siblings ?? []).length > 0) {
      ...
      await ledger.from('transactions')
        .update({ amount: remTotal, name: `${grp?.name ?? 'Chit'} — ${mem?.name ?? ''} — ${list}` })
        .eq('id', coll.income_transaction_id).eq('user_id', owner)
    } else {
      ...
      const { error: txErr } = await ledger.from('transactions')
        .delete().eq('id', coll.income_transaction_id).eq('user_id', owner)
  ...
  const { error: delErr } = await supabase.from('chit_collections')
    .delete().eq('id', coll.id).eq('user_id', owner)
  if (delErr) return NextResponse.json({ error: delErr.message }, { status: 500 })

  // The slot is owed again.
  await supabase.from('chit_receivables')
    .update({ status: 'PENDING', collection_id: null })
    .eq('user_id', owner).eq('group_id', coll.group_id)
