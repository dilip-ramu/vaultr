// Chit staff: who may sign in to your chit module. OWNER ONLY.
//
...
const ROLES: ChitRole[] = ['manager', 'collector', 'viewer']
...
export async function GET() {
  ...
  if (!CAN.manageStaff(access.role)) {
    return NextResponse.json({ error: forbidden('manage staff logins', access.role) }, { status: 403 })
  }

  const supabase = await createClient()
  const { data, error } = await supabase.from('chit_staff')
...
export async function POST(req: NextRequest) {
  ...
  const role = (ROLES.includes(b?.role) ? b.role : 'manager') as ChitRole
  ...
  // Refuse to hand someone access to their own account's data as "staff".
  ...
  const { data: created, error: createErr } = await admin.auth.admin.createUser({
    ...
    user_metadata: { full_name: name, chit_staff_for: access.ownerId },
  })

  let staffUserId = created?.user?.id ?? null
  if (createErr) {
  ...
  if (!staffUserId) return NextResponse.json({ error: 'Could not create that login.' }, { status: 500 })

  const { data, error } = await admin.from('chit_staff').upsert({
    owner_user_id: access.ownerId, staff_user_id: staffUserId,
    name, email, role, is_active: true, must_change_password: true,
    updated_at: new Date().toISOString(),
  }, { onConflict: 'owner_user_id,staff_user_id' }).select('*').single()
...
export async function PATCH(req: NextRequest) {
  ...
  const supabase = await createClient()
  const { data: row } = await supabase.from('chit_staff')
    .select('id, staff_user_id').eq('id', id).eq('owner_user_id', access.ownerId).maybeSingle()
  ...
  if (typeof b?.password === 'string' && b.password.length) {
    ...
    const admin = createAdminClient()
    const { error } = await admin.auth.admin.updateUserById(row.staff_user_id, { password: b.password })
    if (error) return NextResponse.json({ error: error.message }, { status: 500 })
    await admin.from('chit_staff').update({ must_change_password: true }).eq('id', id)
  }
...
export async function DELETE(req: NextRequest) {
  ...
  const supabase = await createClient()
  const { error } = await supabase.from('chit_staff')
