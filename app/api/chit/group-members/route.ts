...
export async function GET(req: NextRequest) {
  ...
  const { data, error } = await supabase.from('chit_group_members')
    ...
    .eq('user_id', owner).eq('group_id', groupId)
...
export async function POST(req: NextRequest) {
  ...
  if (!CAN.manageMembers(access.role)) {
    return NextResponse.json({ error: forbidden('change who is in a chit', access.role) }, { status: 403 })
  }
  ...
  const { data: group } = await supabase.from('chit_groups')
    .select('members').eq('id', groupId).eq('user_id', owner).maybeSingle()
  if (!group) return NextResponse.json({ error: 'Group not found' }, { status: 404 })

  const { data: existing } = await supabase.from('chit_group_members')
    .select('member_id, slot_number').eq('group_id', groupId).eq('user_id', owner)
  ...
  const rows = toAdd.map(member_id => ({
    user_id: owner, group_id: groupId, member_id, slot_number: nextSlot(),
  }))
...
export async function PATCH(req: NextRequest) {
  ...
  const { data: row } = await supabase.from('chit_group_members')
    .select('group_id').eq('id', id).eq('user_id', owner).maybeSingle()
  ...
  const { data, error } = await supabase.from('chit_group_members')
    .update({ slot_number: slot }).eq('id', id).eq('user_id', owner)
...
export async function DELETE(req: NextRequest) {
  ...
  const { error } = await supabase.from('chit_group_members').delete().eq('id', id).eq('user_id', owner)
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
