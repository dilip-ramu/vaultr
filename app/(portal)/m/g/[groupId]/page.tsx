...
import { getGroupDetail, getLiveAuction } from '@/lib/chit/portal-data'
...
export default async function PortalGroupPage({
  ...
  const [detail, auction] = await Promise.all([
    getGroupDetail(memberId, groupId),
    getLiveAuction(memberId, groupId),
  ])
  if (!detail) notFound()
  return <PortalGroup detail={detail} groupId={groupId} auction={auction} />
}
