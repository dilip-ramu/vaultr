...
const SERIES = [
  ...
  { metal: 'platinum', purity: null as string | null, label: 'Platinum', sub: 'pure', emoji: '⚪' },
]
const DERIVED: { label: string; metal: string; frac: number }[] = [
  ...
  { label: 'Platinum 950', metal: 'platinum', frac: 0.95 },
]
    ...
    <div className="max-w-[820px]">
      ...
      {dates.length > 0 && (
        <div className="rounded-2xl overflow-hidden" style={{ border: '1px solid var(--border)' }}>
          <div className="grid px-4 py-2.5" style={{ gridTemplateColumns: '1.3fr 1fr 1fr 1fr', background: 'var(--surface-2)', fontSize: 9.5, fontWeight: 800, letterSpacing: '.06em', color: 'var(--text-faint)' }}>
            <span>DATE</span><span className="text-right">GOLD 24K</span><span className="text-right">SILVER .999</span><span className="text-right">PLATINUM</span>
          </div>
          {dates.slice(0, 12).map((d, i) => (
            <div key={d} className="grid px-4 py-2.5 items-center" style={{ gridTemplateColumns: '1.3fr 1fr 1fr 1fr', borderTop: i ? '1px solid var(--border)' : 'none' }}>
              ...
              <span className="text-[12px] text-right" style={{ color: 'var(--text-muted)', fontVariantNumeric: 'tabular-nums' }}>{fmt(rateAt('platinum', null, d))}</span>
            </div>
          ))}
        </div>
      )}
