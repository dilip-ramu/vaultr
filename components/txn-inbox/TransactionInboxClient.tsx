...
export default function TransactionInboxClient({ drafts: initial, accounts, categories, payees, senders: initialSenders, integration }: Props) {
  ...
  async function fetchNow() {
    if (!integration) { notify('Connect your email first in Email Setup.', 'error'); return }
    ...
    <div className="max-w-3xl mx-auto px-4 py-6 space-y-4">
      ...
      {showSenders && (
        <div className="card p-4 space-y-3">
          ...
          {!integration && <p className="text-xs" style={{ color: 'var(--expense)' }}>No email connected yet — set it up in Settings → Email Setup first.</p>}
        </div>
      )}
