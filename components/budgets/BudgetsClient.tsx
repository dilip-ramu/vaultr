    ...
    <div className={hideHeader ? 'space-y-5' : 'w-full px-4 md:px-8 py-6 space-y-5'}>
      ...
      {overspent.length > 0 && (
        <div
          ...
          style={{ backgroundColor: 'color-mix(in srgb, var(--expense) 8%, transparent)', border: '1px solid color-mix(in srgb, var(--expense) 20%, transparent)' }}
        >
...
function BudgetCard({ budget: b, onEdit, onDelete, onOpen }: {
  ...
  return (
    <div
      ...
      style={{ backgroundColor: 'var(--surface)', border: `1px solid ${overspent ? 'color-mix(in srgb, var(--expense) 30%, transparent)' : 'var(--border)'}` }}
      onClick={() => onOpen(b)}
    >
      <div className="flex items-start justify-between gap-2 mb-3">
        ...
        <div className="flex items-center gap-1 shrink-0">
          {overspent && (
            <span className="px-2 py-0.5 rounded-lg text-[10px] font-bold mr-1" style={{ backgroundColor: 'color-mix(in srgb, var(--expense) 12%, transparent)', color: 'var(--expense)' }}>
              OVER
            </span>
          )}
          <button
