...
import { deriveBooks, type BooksAccount, type BooksTxn, type BooksCategory, type LedgerGroup } from '@/lib/books/derive'
            ...
            {groups.map(g => {
              ...
              return (
                <Fragment key={g}>
                  ...
                  {rows.map(r => (
                    <tr key={r.key} style={{ borderTop: '1px solid var(--border-2, var(--border))' }}>
                      <td className="px-5 py-2" style={{ color: 'var(--text)' }}>{r.name}</td>
