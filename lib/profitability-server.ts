...
import { linesFromRaw, type ProfitLine, type RawProfitData, type RawTxn } from './profitability'
...
/** Profit lines for the current user. Fast path: get_profitability_lines() RPC
 *  (migration_v31). Falls back to JS aggregation if the RPC doesn't exist. */
export async function fetchProfitLines(supabase: SupabaseClient, uid: string): Promise<ProfitLine[]> {
  const { data, error } = await supabase.rpc('get_profitability_lines')
  if (error) return fetchLinesFallback(supabase, uid)
  return (data ?? []).map((r: { kind: string; side: string; source: string; day: string; amount: number | string }) => ({
